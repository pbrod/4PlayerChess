...
from winxpgui import (InitCommonControls, CallWindowProc, CreateWindowEx,
...
class Waitbar(Thread):
    ...
    def BuildWindow(self):
        ...
        self.dialog = CreateWindowEx(
            ...
            int(around(GetSystemMetrics(win32con.SM_CXSCREEN) * .5 - width * .5)),
            int(around(GetSystemMetrics(win32con.SM_CYSCREEN) * .5 - height * .5)),
            width,
            height,
            0,
            0,
    ...
    def run(self):
        ...
        while self.started:
            ...
            if self.do_update:
                ...
                percentage = int(around(100.0 * self.position / self.max_val))
                SendMessage(self.dialog, win32con.WM_SETTEXT, 0,
...
class DoubleWaitbar(Thread):

    def __init__(self, title='DoubleWaitbar', can_abort=True, max_val=100):
        Thread.__init__(self)     # Initialize thread
        self.title = title
        self.can_abort = can_abort
        self.max_val = max_val
        ...
        self.position1 = 0
        self.position2 = 0
        self.do_update1 = False
        self.do_update2 = False
        self.dialog = None
        self.label1 = self.label2 = ''
        self.progbar1 = self.progbar2 = None
        self.button = None
        self.oldWndProc = None
    ...
    def BuildWindow(self):
        ...
        self.oldWndProc = SetWindowLong(
            ...
            win32con.GWL_WNDPROC,
            self.DlgProc)
        SendMessage(self.progbar1, PBM_SETRANGE, 0,
                    MAKELPARAM(0, self.max_val))
        SendMessage(self.progbar2, PBM_SETRANGE, 0,
                    MAKELPARAM(0, self.max_val))
        SendMessage(self.label1, win32con.WM_SETTEXT, 0,
                    'Overall progress (0%)')
        SendMessage(self.label2, win32con.WM_SETTEXT, 0, 'Current task (0%)')
        ShowWindow(self.dialog, win32con.SW_SHOW)
    ...
    def run(self):
        ...
        while self.started:
            PumpWaitingMessages()
            if self.do_update1:
                SendMessage(self.progbar1, PBM_SETPOS,
                            int(self.position1 % self.max_val), 0)
                percentage = int(around(100.0 * self.position1 / self.max_val))
                SendMessage(self.label1, win32con.WM_SETTEXT, 0,
                            'Overall progress (%d%%)' % percentage)
                self.do_update1 = False
            if self.do_update2:
                SendMessage(self.progbar2, PBM_SETPOS,
                            int(self.position2 % self.max_val), 0)
                percentage = int(around(100.0 * self.position2 / self.max_val))
                SendMessage(self.label2, win32con.WM_SETTEXT, 0,
                            'Current task (%d%%)' % percentage)
                self.do_update2 = False
            sleep(0.1)
        PostQuitMessage(0)
        DestroyWindow(self.dialog)
    ...
    def update(self, pos):
        if self.started:
            if not self.do_update1:
                self.position1 = pos
                self.do_update1 = True
            return True
        return False

    def update2(self, pos):
        if self.started:
            if not self.do_update2:
                self.position2 = pos
                self.do_update2 = True
            return True
        return False
...
class InfoDlg(Thread):

    def __init__(self, message='', title='Information', blocking=False):
        Thread.__init__(self)     # Initialize thread
        self.title = title
...
class WarnDlg(Thread):
    ...
    def __init__(self, message='', title='Warning!', blocking=False):
        Thread.__init__(self)     # Initialize thread
        self.title = title
        self.message = message
        if blocking:
...
if __name__ == '__main__':
    ...
    wb = DoubleWaitbar('Waitbar example')
    for i in range(5):
        print(wb.update(i * 20))
        for j in range(20):
            wb.update2(j * 5)
            sleep(0.1)
    wb.close()
