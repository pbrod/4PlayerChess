...
class View(QWidget):
    ...
    def squareCenter(self, square, orientation=None):
        ...
        file = square.x()
        rank = square.y()
        return self.squareCenter2(file, rank, orientation)

    def squareCenter2(self, file, rank, orientation=None):
    ...
    def drawCoordinates(self, painter):
        for y in range(14):
            ...
            file, rank = self._get_file_and_rank(y, x)
            palette = QPalette.Light if (file + rank) % 2 else QPalette.Dark
            color = self.palette().color(palette)
    ...
    def drawCoordinateHelp(self, painter):
        if self.coordinate:
    ...
    def drawSquaresAndHighlight(self, painter):
    # Draw squares
        for rank in range(self.board.ranks):
            for file in range(self.board.files):
    ...
    def paintEvent(self, event):
        ...
        self.drawSquaresAndHighlight(painter)
        ...
        if SETTINGS.value('coordinatehelp', type='bool'):
            self.drawCoordinateHelp(painter)
        painter.end()
    ...
    def drawSquare(self, painter, file, rank):
        ...
        palette = QPalette.Midlight if (file + rank) % 2 else QPalette.Mid
        fillColor = self.palette().color(palette)
