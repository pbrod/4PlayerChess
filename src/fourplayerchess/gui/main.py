...
from PyQt5.QtGui import QIcon, QFont, QFontMetrics, QPainter, QDesktopServices  # , QColor
from fourplayerchess.ui.mainwindow import Ui_MainWindow
from fourplayerchess.ui.settings import Ui_Preferences
from fourplayerchess.ui.infodialog import Ui_InfoDialog
...
GAME_ROOT = os.path.join(ROOT, "data", "games")
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        pieces = ['rP', 'rN', 'rR', 'rB', 'rQ', 'rK',
                  ...
                  'yP', 'yN', 'yR', 'yB', 'yQ', 'yK',
                  'gP', 'gN', 'gR', 'gB', 'gQ', 'gK']
        root_piece = os.path.join(ROOT, 'resources', 'img', 'pieces')
        for piece in pieces:
            self.view.setPiece(piece, QIcon(os.path.join(root_piece, piece + '.svg')))
        ...
        self.algorithm.addArrow.connect(self.addArrow)
    ...
    def addArrow(self, fromFile, fromRank, toFile, toRank, color):
        """Adds arrows to board view."""
        v = self.view
        orientation = v.orientation[0]
        origin = v.squareCenter2(fromFile, fromRank, orientation)
        target = v.squareCenter2(toFile, toRank, v.orientation)
        arrow = v.Arrow(origin, target, color)
        self.view.addHighlight(arrow)

    def viewClicked(self, square):
        """Handles view click event to move clicked piece to clicked square."""
        color = self.algorithm.currentPlayerColor
    ...
    def movePiece(self, fromSquare, toSquare):
        ...
        if not moved:
            ...
            self.view.highlightChecks()
            # Remove highlights of next player
            self.view.removeHighlightsOfColor(self.algorithm.currentPlayerColor)
            self.moveHighlight = 0
        self.selectedSquare = 0
...
""")
    ...
    def openFileNameDialog(self):
        ...
        fileName, _ = QFileDialog.getOpenFileName(self, "Load Game", GAME_ROOT,
                                                  "PGN4 Files (*.pgn4)", options=options)
        if fileName:
            with open(fileName, 'r') as file:
                pgn4 = file.read()
            self.pgnField.setPlainText(pgn4)
            loaded = self._parcePgn4(pgn4)
            if loaded:
    ...
    def saveFileDialog(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self, "Save Game", GAME_ROOT,
                                                  "PGN4 Files (*.pgn4)", options=options)
        if fileName:
            ext = '.pgn4'
